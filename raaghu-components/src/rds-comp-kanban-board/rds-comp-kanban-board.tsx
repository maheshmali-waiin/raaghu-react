...
const RdsCompKanbanBoard = (props: RdsCompKanbanBoardProps) => {
  ...
  >(props.boardData);
    ...
    const [subCardInputsVisible, setSubCardInputsVisible] =
    useState<number | null>(props.boardData ? props.boardData.length : 0);
    ...
    const deleteSubCard = (index: number, subCardIndex: number) => {
        ...
        props.boardData.map((board: any) => {
            if (board.subCardIndex === index) {
                return {
                    ...board,
                    subCards: board.subCards.filter((subCard: any) => subCard !== cardToDelete),
                };
            }
            return board;
        });        
